...
import { AlertCircle, Zap, Calendar as CalendarIcon, X, ChevronLeft, ChevronRight, Layers } from 'lucide-react';
...
const MobileAttendance = ({ data, schedule }) => {
  ...
  const baseAttendance = useMemo(() => {
    return rawAttendance.map((subject, index) => {
  ...
  const overallStats = useMemo(() => {
      ...
      let category = overallPct < 75 ? 'cooked' : overallPct >= 85 ? 'safe' : 'danger';
  ...
  const getImpactMap = () => {
    ...
    selectedDates.forEach(dateStr => {
        ...
        if (dayInfo && dayInfo.order && dayInfo.order !== '-' && !isNaN(parseInt(dayInfo.order))) {
            const dayOrderKey = `Day ${parseInt(dayInfo.order)}`;
            const dayClasses = effectiveSchedule[dayOrderKey] || effectiveSchedule[`Day ${dayInfo.order}`];
            ...
            if (dayClasses) {
                Object.values(dayClasses).forEach((cls) => {
                    ...
                    const matchedSubject = baseAttendance.find(s => {
                        ...
                        return sName === cName || (sName.length > 4 && cName.length > 4 && sName.includes(cName));
                    });

                    if (matchedSubject) {
                         impact[matchedSubject.code] = (impact[matchedSubject.code] || 0) + 1;
                    }
                });
            }
        }
  ...
  const predictionImpact = useMemo(() => getImpactMap(), [selectedDates, calendarData, effectiveSchedule, baseAttendance]);

  const getPredictionForSubject = (subject) => {
      ...
      const currentStatus = getStatus(parseFloat(subject.percentage), currentConducted, currentPresent);
  ...
  const handleDateClick = (day) => {
      const clickedDate = new Date(calMonth.getFullYear(), calMonth.getMonth(), day);
      const dateKey = formatDateKey(clickedDate);

      if (isRangeMode) {
          if (!rangeStart) {
              setRangeStart(clickedDate);
              if (!selectedDates.includes(dateKey)) setSelectedDates([...selectedDates, dateKey]);
          } else {
  ...
  const activeSubject = baseAttendance.find(s => s.id === selectedId) || baseAttendance[0];
  const currentActiveStat = getStatus(parseFloat(activeSubject.percentage), activeSubject.conducted, activeSubject.present);
  
  const bgColor = predictMode 
    ? '#f0f0f0' 
    : (currentActiveStat.safe ? '#ceff1c' : '#ff003c');
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#f5f6fc] text-[#050505] font-sans relative overflow-hidden touch-pan-y">
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {introMode ? (
            ...
            >
              <h1 className="text-6xl font-black lowercase tracking-tighter text-[#050505] mb-2" style={{ fontFamily: 'Aonic' }}>{overallStats.badge}</h1>
              <p className="text-xl font-bold lowercase text-[#050505] leading-tight max-w-[80%]" style={{ fontFamily: 'Aonic' }}>{overallStats.tagline}</p>
            </motion.div>
          ) : !predictMode ? (
            <motion.div
              key="dashboard"
            ...
            >
               <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2 px-3 py-1 bg-black/5 rounded-full border border-black/5 backdrop-blur-sm">
                    {currentActiveStat.safe ? <Zap size={12} fill="black" /> : <AlertCircle size={12} fill="black" stroke="black" />}
                    <span className="font-mono text-[10px] lowercase tracking-widest font-bold text-black/60">{activeSubject.badge}</span>
                  </div>
                  <button onClick={() => setPredictMode(true)} className="flex items-center gap-2 bg-black text-white px-4 py-2 rounded-full active:scale-95 transition-transform shadow-lg">
               ...
               <div className="my-auto flex flex-col justify-center">
                  <div className="flex items-baseline gap-2">
                    <span className={`text-[22vw] md:text-[9rem] leading-none font-black tracking-tighter ${currentActiveStat.textColor}`} style={{ fontFamily: 'Urbanosta' }}>
                      <MarginCounter value={currentActiveStat.val} color={currentActiveStat.textColor} />
                      h
                    </span>
                  </div>
               </div>
               ...
               <div className="pb-1">
                  <h3 className={`text-2xl md:text-3xl font-bold lowercase leading-tight mb-3 line-clamp-1 ${currentActiveStat.textColor}`} style={{ fontFamily: 'Aonic' }}>{activeSubject.title.toLowerCase()}</h3>
                  <div className="w-full h-[4px] bg-black/10 mb-2 relative overflow-hidden rounded-full">
                    <motion.div className={`h-full ${currentActiveStat.lineColor}`} initial={{ width: 0 }} animate={{ width: `${Math.min(parseFloat(activeSubject.percentage), 100)}%` }} transition={{ duration: 0.8, ease: "circOut" }} />
                  </div>
                  <span className={`block text-[10px] font-mono font-bold lowercase mt-1 ${currentActiveStat.textColor} opacity-60`}>{activeSubject.present}/{activeSubject.conducted} sessions</span>
               </div>
            </motion.div>
          ) : (
            <motion.div
            ...
            >
                <div className="flex justify-between items-center p-6 pb-0">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => setPredictMode(false)} className="p-2 bg-black/10 hover:bg-black/20 rounded-full transition-colors"><X size={16} /></button>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col px-6 pb-4 pt-2 overflow-hidden">
                    <div className="bg-white/60 backdrop-blur-md rounded-3xl p-3 mb-2 flex-1 flex flex-col min-h-0 shadow-sm border border-black/5">
                        <div className="flex justify-between items-center mb-2 px-2">
                            ...
                            <span className="font-bold uppercase tracking-wide text-xs" style={{ fontFamily: 'Aonic' }}>{calMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
                            <button onClick={() => changeMonth(1)} className="p-1 hover:bg-black/5 rounded-full"><ChevronRight size={16} /></button>
                        </div>
                        
                        <div className="grid grid-cols-7 text-center mb-1">
                            {['M','T','W','T','F','S','S'].map((d, i) => <span key={i} className="text-[9px] font-bold text-black/30">{d}</span>)}
                        </div>
                        
                        <div className="grid grid-cols-7 gap-y-1 justify-items-center overflow-y-auto custom-scrollbar">
                            {Array.from({ length: new Date(calMonth.getFullYear(), calMonth.getMonth(), 1).getDay() === 0 ? 6 : new Date(calMonth.getFullYear(), calMonth.getMonth(), 1).getDay() - 1 }).map((_, i) => <div key={`e-${i}`} />)}
                            {Array.from({ length: new Date(calMonth.getFullYear(), calMonth.getMonth() + 1, 0).getDate() }).map((_, i) => {
                                ...
                                const isRangeStart = rangeStart && rangeStart.toDateString() === dateObj.toDateString();
                                ...
                                return (
                                    <button 
                                        ...
                                        className={`w-8 h-8 flex items-center justify-center rounded-full text-[10px] font-bold transition-all relative 
                                            ${isSelected ? 'bg-[#050505] text-[#ceff1c] scale-105 shadow-md z-10' : 
                                              isRangeStart ? 'bg-black/50 text-white' : 
                                              isToday ? 'bg-transparent text-[#050505] ring-2 ring-[#050505]' : 'text-black/60 hover:bg-black/5'}`}
                                    >
                                        {d}
                                    </button>
                                );
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        <div className="flex bg-white shadow-sm border border-black/5 p-1 rounded-full w-full max-w-[200px] relative h-10">
                            <motion.div layoutId="predToggle" className={`absolute rounded-full h-[calc(100%-8px)] top-1 ${predType === 'attend' ? 'bg-[#050505]' : 'bg-[#ff003c]'}`} style={{ width: 'calc(50% - 4px)', left: predType === 'attend' ? 4 : '50%' }} />
                            <button onClick={() => setPredType('attend')} className={`flex-1 text-[10px] font-black uppercase tracking-wider relative z-10 transition-colors ${predType === 'attend' ? 'text-white' : 'text-black/40'}`}>Attend</button>
                            <button onClick={() => setPredType('leave')} className={`flex-1 text-[10px] font-black uppercase tracking-wider relative z-10 transition-colors ${predType === 'leave' ? 'text-white' : 'text-black/40'}`}>Leave</button>
                        </div>
                        <div className="flex-1 flex justify-end items-center">
                             <div className="flex flex-col items-end">
                                <span className="text-2xl font-black leading-none" style={{ fontFamily: 'Urbanosta' }}>{selectedDates.length}</span>
                                <span className="text-[8px] font-bold uppercase opacity-50">Sessions</span>
                             </div>
                        </div>
                    </div>
                </div>
      ...
      <div 
        ...
        style={{ touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', overflowAnchor: 'none' }}
      >
        <div className="px-6 py-6 flex flex-col gap-4">
          <span className="font-mono text-[10px] lowercase tracking-widest text-[#050505]/40 mb-2 block sticky top-0 bg-[#f5f6fc] z-10 py-2">/// {predictMode ? 'predicted margin' : 'watchlist'}</span>
              ...
              >
                <div className="flex justify-between items-end mb-3">
                  ...
                  <div className="flex flex-col items-end min-w-[80px]">
                      <span className={`text-2xl font-black leading-none ${predictMode ? (isSafe ? 'text-[#050505]' : 'text-[#ff003c]') : (currentActiveStat.safe ? 'text-[#050505]' : 'text-[#ff003c]')}`} style={{ fontFamily: 'Urbanosta' }}>
                          {predictMode ? predData.status.val : Math.floor(parseFloat(subject.percentage)) + '%'}
                      </span>
                      
                      {predictMode && (
                          <div className="flex flex-col items-end mt-1">
                              <span className={`text-[10px] font-bold lowercase ${isSafe ? 'text-[#050505]' : 'text-[#ff003c]'}`}>
                ...
                <div className="w-full h-[2px] bg-[#050505]/5 relative mb-3 rounded-full overflow-hidden">
                  {predictMode && <div className="h-full absolute top-0 left-0 bg-black/10" style={{ width: `${Math.min(parseFloat(subject.percentage), 100)}%` }} />}
                  <div className={`h-full absolute top-0 left-0 transition-all duration-500 ${predictMode ? (isSafe ? 'bg-[#050505]' : 'bg-[#ff003c]') : (currentActiveStat.safe ? 'bg-[#050505]' : 'bg-[#ff003c]')}`} style={{ width: `${Math.min(predictMode ? predData.pct : parseFloat(subject.percentage), 100)}%` }} />
                </div>

                <div className="flex justify-between items-center text-[10px] font-mono tracking-wide text-[#050505]/50 lowercase">
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${predictMode ? (isSafe ? 'bg-[#ceff1c]' : 'bg-[#ff003c]') : (currentActiveStat.safe ? 'bg-[#ceff1c]' : 'bg-[#ff003c]')}`} />
                    <span>{subject.code.toLowerCase()}</span>
                  </div>
                  {predictMode && <span className="font-bold flex items-center gap-1">{affected && <span className="w-1 h-1 bg-black rounded-full animate-pulse"/>}{Math.floor(predData.pct)}%</span>}
                </div>
              </div>
            );
          })}
