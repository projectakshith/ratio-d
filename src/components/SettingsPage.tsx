...
import { 
  ChevronLeft, Pencil, Share2, Bell, Palette, Lock, Cloud, 
  ChevronRight, LogOut, Check, TestTube, Sparkles
} from 'lucide-react';
...
const panelVariants = {
  hidden: { x: '-100%' },
  visible: { x: '0%', transition: { duration: 0.7, ease: [0.6, 0.05, 0.01, 0.9], when: 'beforeChildren', staggerChildren: 0.05 } },
  exit: { x: '-100%', transition: { duration: 0.4, ease: 'easeIn' } }
};

const themePanelVariants = {
  hidden: { x: '100%' },
  visible: { x: '0%', transition: { duration: 0.5, ease: [0.6, 0.05, 0.01, 0.9], when: 'beforeChildren', staggerChildren: 0.05 } },
          ...
          <div className="px-6 py-8 space-y-12">
                  ...
                  {!isEditing ? (
                    ...
                    <motion.div key="input-stack" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="flex flex-col gap-3">
                      ...
                      <div className="flex gap-2">
                        <button onClick={handleSave} className="flex-1 py-3 rounded-[22px] bg-white text-black font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform">
                          <Check className="w-4 h-4" /> Save
                        </button>
                        <button onClick={() => { setIsEditing(false); setTempName(""); }} className="px-6 py-3 rounded-[22px] bg-white/5 text-white/60 text-sm font-semibold active:scale-95 transition-transform">
                          Cancel
                        </button>
                      </div>
                    </motion.div>
            ...
            <motion.div variants={itemVariants} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                {onTestNotification && (
                  <SettingItem
                    icon={<TestTube className="w-5 h-5 opacity-80 text-[#ceff1c]" />}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-6">
                <div className="space-y-1">
                  {THEMES.map((theme, index) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                    >
                      <span className="text-[17px] font-medium tracking-tight">{theme.name}</span>
                    </motion.button>
                  ))}
                </div>
                ...
                >
                  ...
                  <span className="text-xs italic">more themes coming soon</span>
                </motion.div>
              </div>
            </motion.div>
          </>
...
const SettingItem = ({ icon, label, toggle = false, isActive = false, onClick, value }) => (
